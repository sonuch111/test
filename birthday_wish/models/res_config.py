...
class ResConfigSettings(models.TransientModel):
    ...
    def get_values(self):
        ...
        res.update(
            birthday_mail_template=self.env['ir.config_parameter'].sudo().get_param('birthday_mail_template')
        )
        return res

    def set_values(self):
        super(ResConfigSettings, self).set_values()
        self.env['ir.config_parameter'].sudo().set_param('birthday_mail_template', self.birthday_mail_template.id)
